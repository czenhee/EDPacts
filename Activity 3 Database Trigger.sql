...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_age` AFTER INSERT ON `agents` FOR EACH ROW BEGIN
	if new.Players_ID is not null then
		update players_team
		set PAge = PAge + newPAge
		where Player_ID = new.Players_ID;
    end if;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_age2` AFTER UPDATE ON `agents` FOR EACH ROW BEGIN
	if old.Players_ID is not null then
		update players_team
        set PAge = PAge - oldPAge
        where Player_ID = old.Players_ID;
	end if;
    if new.Players_ID is not null then
		update players_team
        set PAge = PAge + newPAge
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_age3` BEFORE DELETE ON `agents` FOR EACH ROW BEGIN
	if (old.Players_ID is not null) then
    update players_team
    set age = age - oldage
    where Player_ID = old.Players_ID;
...
-- Dump completed on 2023-03-22 19:14:37
